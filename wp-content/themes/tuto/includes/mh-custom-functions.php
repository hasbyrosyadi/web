...
if (!function_exists('tuto_add_body_class')) {
	function tuto_add_body_class($classes) {
		$tuto_options = tuto_theme_options();
		$classes[] = 'mh-' . esc_attr($tuto_options['sb_position']) . '-sb';
		return $classes;
	}
}
	...
	function tuto_custom_header() {
		echo '<div class="mh-custom-header">' . "\n";
			if (get_header_image()) {
				...
				echo '</a>' . "\n". '<div style="float:left; padding-right: 70px"><img src="/wordpress/wp-includes/images/logo.png" width="650" height="750"></div> ';
			}
